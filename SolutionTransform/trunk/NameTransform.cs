...
namespace SolutionTransform
{
	...
	public class NameTransform : MSBuild2003Transform
	{
		private readonly Func<string, string> nameReplace;

		public NameTransform(Func<string, string> nameReplace)
		{
		...
		public override void DoApplyTransform(XmlDocument document)
		{
			...
			{
				element.SetAttribute("Include", nameReplace(element.GetAttribute("Include")));
			}
			foreach (XmlElement element in document.SelectNodes("//x:ProjectReference/x:Name", namespaces)) {
				element.InnerText = nameReplace(element.InnerText);
			}
		}
