...
namespace Castle.Components.Scheduler.JobStores
{
    ...
    public abstract class AdoNetJobStoreDao : IJobStoreDao
    {
        ...
        public virtual void RegisterScheduler(string clusterName, Guid schedulerGuid, string schedulerName, DateTime lastSeenUtc)
        {
            try
            {
        ...
        /// <exception cref="SchedulerException">Thrown if an error occurs</exception>
        public virtual bool CreateJob(string clusterName, JobSpec jobSpec, DateTime creationTimeUtc, CreateJobConflictAction conflictAction)
        {
            try
            {
                using (IDbConnection connection = CreateConnection())
                {
                    ...
                    AddInputParameter(command, "TriggerObject", DbType.Binary, DbUtils.MapObjectToDbValue(DbUtils.SerializeObject(jobSpec.Trigger)));
                    AddInputParameter(command, "JobDataObject", DbType.Binary, DbUtils.MapObjectToDbValue(DbUtils.SerializeObject(jobSpec.JobData)));
                    ...
                    switch (resultCode)
                    {
                        ...
                        default:
                            throw new SchedulerException(String.Format(CultureInfo.CurrentCulture,
                                "spSCHED_CreateJob returned unrecognized result code '{0}'.", resultCode));
                    }
                }
            }
            catch (Exception ex)
        ...
        public virtual void UpdateJob(string clusterName, string existingJobName, JobSpec updatedJobSpec)
        {
            try
            {
                using (IDbConnection connection = CreateConnection())
                {
                    ...
                    AddInputParameter(command, "UpdatedTriggerObject", DbType.Binary, DbUtils.MapObjectToDbValue(DbUtils.SerializeObject(updatedJobSpec.Trigger)));
                    AddInputParameter(command, "UpdatedJobDataObject", DbType.Binary, DbUtils.MapObjectToDbValue(DbUtils.SerializeObject(updatedJobSpec.JobData)));
                    IDbDataParameter resultCodeParam = AddOutputParameter(command, "ResultCode", DbType.Int32);
                    ...
                    switch (resultCode)
                    {
                        ...
                        default:
                            throw new SchedulerException(String.Format(CultureInfo.CurrentCulture,
                                "spSCHED_UpdateJob returned unrecognized result code '{0}'.", resultCode));
                    }
                }
            }
            catch (Exception ex)
        ...
        public virtual void SaveJobDetails(string clusterName, VersionedJobDetails jobDetails)
        {
            try
            {
                using (IDbConnection connection = CreateConnection())
                {
                    ...
                    AddInputParameter(command, "TriggerObject", DbType.Binary, DbUtils.MapObjectToDbValue(DbUtils.SerializeObject(jobDetails.JobSpec.Trigger)));

                    AddInputParameter(command, "JobDataObject", DbType.Binary, DbUtils.MapObjectToDbValue(DbUtils.SerializeObject(jobDetails.JobSpec.JobData)));
                    AddInputParameter(command, "JobState", DbType.Int32, jobDetails.JobState);
                    AddInputParameter(command, "NextTriggerFireTime", DbType.DateTime, DbUtils.MapNullableToDbValue(jobDetails.NextTriggerFireTimeUtc));
                    ...
                    AddInputParameter(command, "NextTriggerMisfireThresholdSeconds", DbType.Int32, DbUtils.MapNullableToDbValue(nextTriggerMisfireThresholdSeconds));
                    ...
                    if (!wasSaved)
                        throw new ConcurrentModificationException(String.Format("Job '{0}' does not exist or was concurrently modified in the database.",
            ...
            int schedulerExpirationTimeInSeconds, out DateTime? nextTriggerFireTimeUtc)
        {
            try
            {
                using (IDbConnection connection = CreateConnection())
                {
                    ...
                    nextTriggerFireTimeUtc = DateTimeUtils.AssumeUniversalTime(DbUtils.MapDbValueToNullable<DateTime>(nextTriggerFireTimeParam.Value));
                    return jobDetails;
                }
            }
            catch (Exception ex)
        ...
        protected virtual VersionedJobDetails BuildJobDetailsFromResultSet(IDataReader reader)
        {
            ...
            DateTime? nextTriggerFireTimeUtc = DateTimeUtils.AssumeUniversalTime(DbUtils.MapDbValueToNullable<DateTime>(reader.GetValue(7)));
            ...
            DateTime? lastExecutionStartTimeUtc = DateTimeUtils.AssumeUniversalTime(DbUtils.MapDbValueToNullable<DateTime>(reader.GetValue(10)));
            DateTime? lastExecutionEndTimeUtc = DateTimeUtils.AssumeUniversalTime(DbUtils.MapDbValueToNullable<DateTime>(reader.GetValue(11)));
            ...
            {
                JobExecutionDetails execution = new JobExecutionDetails(lastExecutionSchedulerGuid.Value, lastExecutionStartTimeUtc.Value);
