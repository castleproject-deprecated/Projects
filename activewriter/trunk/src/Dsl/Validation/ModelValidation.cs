...
namespace Castle.ActiveWriter
{
    ...
    public partial class Model
    {
        ...
        {
            if (!UseVirtualProperties && Classes.Count > 0)
            {
                foreach (ModelClass cls in Classes)
                {
                    if (cls.Lazy)
        ...
        {
            if (Target == CodeGenerationTarget.NHibernate && string.IsNullOrEmpty(ActiveRecordAssemblyName))
            {
        ...
        {
            if (Target == CodeGenerationTarget.NHibernate && string.IsNullOrEmpty(NHibernateAssemblyName))
            {
